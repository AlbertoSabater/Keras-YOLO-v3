...
#            images, box = f(annotation_lines[i], input_shape, random=random)
        ...
        try:
            ...
            print(i)
            for l in annotation_lines[i-batch_size:i]: print(l)
            raise(ValueError(''))
        yield [image_data, *y_true], np.zeros(batch_size)
...
#    annotations_r = './dataset_scripts/adl/annotations_adl_val_r_fd10_fsn1.txt'
    ...
    generator = data_generator_wrapper_custom(annotation_lines[2000:], 32, input_shape, anchors, num_classes, random=True)
